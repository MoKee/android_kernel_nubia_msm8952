...
{    
	...
	struct smbchg_chip *chip = container_of(psy, struct smbchg_chip, batt_psy);	
	int ret = 0;		
...
static ssize_t show_charge_full_design(struct device *dev,	struct device_attribute *attr, char *buf)
{    
	struct power_supply	 *psy = dev_get_drvdata(dev);	
	struct smbchg_chip *chip = container_of(psy, struct smbchg_chip, batt_psy);	
	int ret = 0;	
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	#ifdef CONFIG_ZTEMT_COMMON_CHARGER
	if (usb_supply_type == POWER_SUPPLY_TYPE_USB && current_limit == 100)
		current_limit = 500;
	#endif
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	#ifdef CONFIG_ZTEMT_COMMON_CHARGER
    of_property_read_u32(chip->spmi->dev.of_node,
					"qcom,full-design-mah",
					&chip->batt_full_design_mah);
	chip->batt_warm_cntl = of_property_read_bool(node, 
